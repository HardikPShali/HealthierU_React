...
const Welcome = ({ currentuserInfo }) => {
    ...
    const [
        currentUserDataAfterApproval,
        setCurrentUserDataAfterApproval,
    ] = useState({});
    ...
    const loadDocuments = async () => {
        ...
        if (profileStatus) {
            const currentUserData = async () => {
                const res = await getCurrentDoctorInfo(
                    currentuserInfo.id,
    ...
    const getUpdatedCurrentUserData = async () => {
        if (
            ...
            if (
                ...
                currentUserInformation.data.profileCompleted
            ) {
        ...
        if (
            ...
            if (documentInfo && documentUpdateFile) {
                ...
                if (
                    ...
                    currentUserInformation.data &&
                    currentUserInformation.data.profileCompleted &&
                    !currentUserInformation.data.approved
                ) {
    ...
    const handleDetails = async (e) => {
        ...
        if (
            ...
            if (languages.length === 0) {
                ...
                const response = await updateRoleDoctor(bodyFormDataDoctor).catch(
                    (err) => {
                        setTransparentLoading(false);
                        if (err.response.status === 400 && state.phone === '') {
                            setPhoneError(err.response.data.title);
        ...
        <div>
                ...
                <Row>
                    ...
                    <Col md={6} style={{ background: '#fff', padding: '2%' }}>
                        <div className="sign-box signup-form-helper">
                            <h2 id="welcome-title">Tell Us More About You!</h2>
                            <br />
                            {!displaydocumentForm && (
                                <>
                                    <div>
                                        ...
                                        <Row>
                                            ...
                                            <small className="left">
                                                By providing your mobile number, you give us permission
                                                to contact you via text. View terms.
                                            </small>
                                            {currentuserInfo &&
                                                currentuserInfo.authorities.some(
                                                    (user) => user === 'ROLE_PATIENT'
                            ...
                            {displaydocumentForm && (
                                <>
                                    ...
                                    <button
                                        className="btn btn-primary continue-btn"
                                        onClick={() => getUpdatedCurrentUserData()}
                                    >
                                        Continue
                                    </button>
                                </>
            ...
            <Dialog aria-labelledby="customized-dialog-title" open={open}>
                        ...
                        {currentuserInfo &&
                            ...
                            currentuserInfo.authorities.some(
                                ...
                                <Typography gutterBottom>
                                    You have successfully complete your profile details. Please
                                    click Ok to proceed.
                                </Typography>
                            )}
                        {currentuserInfo &&
                            ...
                            currentuserInfo.authorities.some(
                                ...
                                <Typography gutterBottom>
                                    You have successfully complete your profile details.But admin
                                    approval is pending for account activation. Please contact the
                                    administrator.
                                </Typography>
                            )}
                    </>
                ...
                <DialogActions>
                    <>
                        {currentUserDataAfterApproval &&
                            Object.keys(currentUserDataAfterApproval).length > 0 &&
                            currentUserDataAfterApproval.authorities.some(
                                (user) => user === 'ROLE_PATIENT'
                            ) &&
                            currentUserDataAfterApproval.profileCompleted && (
                                <Link to="/patient/questionnaire/new">
                                    <button
                                        autoFocus
                                        onClick={handleClose}
                        ...
                        {currentUserDataAfterApproval &&
                            ...
                            currentUserDataAfterApproval.profileCompleted &&
                            !currentUserDataAfterApproval.approved && (
